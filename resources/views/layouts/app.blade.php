...
<body>
                ...
                <div class="logo">
                    ...
                    <span>
                        @if (auth()->user()->organization->image)
                            <img src="{{ asset('storage/' . auth()->user()->organization->image) }}"
            ...
            <div class="header-main-right">
                ...
                <nav class="navbar navbar-expand-lg">
                    <button class="navbar-toggler" type="button" data-bs-toggle="collapse"
                        data-bs-target="#responsive-navbar-nav" aria-controls="responsive-navbar-nav"
                    ...
                    <div class="collapse navbar-collapse justify-content-end" id="responsive-navbar-nav">
                        <ul class="navbar-nav align-items-center">
                            ...
                            <!-- Roles & Permission -->
                            <li class="nav-item">
                                <a class="nav-link" wire:navigate href="{{ route('role.index') }}"
                                    data-bs-toggle="tooltip" data-bs-placement="top"
                                    data-bs-custom-class="custom-tooltip" data-bs-title="Roles & Permission"><i
                                        class='bx bx-slider-alt'></i></a>
                            </li>

                            <!-- Login -->
                            <li class="nav-item navbar-dropdown dropdown">
                                <a href="#" class="nav-link dropdown-toggle hide-arrow" href="javascript:void(0);"
                                ...
                                <ul class="dropdown-menu dropdown-menu-end dropdown-account" data-bs-popper="static">
                                    ...
                                    <li>
                                        <a class="dropdown-item align-items-center" wire:navigate
                                    ...
                                    <li>
                                        <a class="dropdown-item align-items-center" wire:navigate
                                            href="{{ route('developers.dashboard') }}">
                                    ...
                                    <li>
                                        <a class="dropdown-item align-items-center text-danger"
                                            href="{{ route('logout') }}">
    ...
    <div class="main-body">
                ...
                <ul class="list-none menu-sidebar">
                    <li>
                        <a wire:navigate href="{{ route('dashboard') }}"
                            class="@if (request()->routeIs('dashboard')) active @endif"><i class='bx bx-line-chart'></i>
                            Dashboard</a>
                    </li>
                    <hr class="my-1">
                    <li>
                    <li>
                        <a wire:navigate
                            href="{{ route('client.profile', Auth::user()->organization->mainClient->id) }}"
                            class="@if (request()->segment(4) == Auth::user()->organization->mainClient->id &&
                                    (request()->segment(2) == 'clients' || request()->segment(2) == 'client')) active @endif">
                            <i class='bx bx-body'></i>
                            Organization
                            {{-- <small>{{ ucfirst(Auth::user()->organization->name) }}</small> --}}
                        </a>
                    </li>
                    </li>
                    ...
                    @can('View Team')
                        <li
                            @if (isset($tour['main_tour'])) data-step="1" data-position='right' data-intro='Create your first team' @endif>
                            <a wire:navigate href="{{ route('team.index') }}"
                                class="@if (request()->segment(2) == 'teams' || request()->segment(2) == 'team') active @endif"><i class='bx bx-sitemap'></i>
                                Teams</a></li>
                    @endcan
                    @can('View User')
                        <li
                            @if (isset($tour['main_tour'])) data-step="2" data-position='right' data-intro='Create your first user' @endif>
                            <a wire:navigate href="{{ route('user.index') }}"
                                class="@if (request()->segment(2) == 'users' || request()->segment(2) == 'user') active @endif"><i class='bx bx-user'></i>
                                Users</a></li>
                    @endcan
                    <hr class="my-1">
                    @if (session('for') == 'agency')
                        @can('View Client')
                            <li
                                @if (isset($tour) && $tour != null) data-step="3" data-intro='Create your first client' data-position='right' @endif>
                                <a wire:navigate href="{{ route('client.index') }}"
                                    class="@if (request()->segment(4) != Auth::user()->organization->mainClient->id &&
                                            (request()->segment(2) == 'clients' || request()->segment(2) == 'client')) active @endif"><i
                                        class='bx bx-briefcase-alt-2'></i> Clients</a></li>
                        @endcan
                    @endif
                    @can('View Project')
                        <li
                            @if (isset($tour['main_tour'])) data-step="4" data-position='right' data-intro='Create your first project' @endif>
                            <a wire:navigate href="{{ route('project.index') }}"
                                class="@if (request()->segment(2) == 'projects' || request()->segment(2) == 'project') active @endif"><i
                                    class='bx bx-objects-horizontal-left'></i> Projects</a></li>
                    @endcan
                    @can('View Task')
                        <li
                            @if (isset($tour['main_tour'])) data-step="5" data-position='right' data-intro='Create your first task' @endif>
                            <a wire:navigate href="{{ route('task.index') }}"
                                class="@if (request()->segment(2) == 'tasks' || request()->segment(2) == 'task') active @endif"><i
                                    class='bx bx-task'></i>Tasks</a></li>
                    @endcan
                </ul>
            </div>
        ...
        <div class="main-body-content scrollbar scrollbar-primary">
            {{ $slot }}
            <p class="alert alert-warning" wire:offline> Whoops, your device has lost connection. The web page you are
                viewing is offline. </p>
        </div>
        ...
        $(".image_upload_input").change(function() {
            if (this.files && this.files[0]) {
                ...
                reader.onload = function(e) {
                    ...
                    $('.image-preview-section').html('<img src="' + e.target.result +
                        '" alt="Image Preview" class="img-fluid" style="max-width: 200px;">');
                }
